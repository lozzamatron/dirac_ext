// Assembles the Agent Map snapshot for a task from its on-disk records instead of
// from the webview's messages. This is the only way to see what the message
// transcript cannot show: a Goal's grandchildren (a child task's own subagent
// runs) and any run whose cards have left the transcript.
//
// This is its own module because it now has TWO readers — the per-task overlay's
// `getAgentMap` RPC and the fleet map's per-instance card builder (WP5b) — and two
// readers of the same records that each carry their own copy of the assembly will
// drift: the fleet map would quietly disagree with the overlay about the same
// conversation. There is one truth here; both surfaces call this function for it.
//
...
// kind" and the builder always resolves with a valid snapshot. A rejected build
// would leave whichever surface asked with nothing to draw; a root-only snapshot
...
// The fields this builder actually needs from goal.json. Narrowing to a local
 ...
 * are ordinary and contribute "no data of that kind"; the builder always resolves
 * with a valid snapshot.
 *
 * Called by the per-task overlay's `getAgentMap` RPC and, one instance at a time,
 * by the fleet map's snapshot builder — the two must never grow their own copies
 * of this assembly, or they will drift apart about the same conversation.
 * @param taskId The task id to build the map for
 * @returns The snapshot object itself; callers serialize it as they need
 */
export async function buildAgentMapSnapshot(taskId: string): Promise<AgentMapSnapshot> {
	...
	if (id === "") {
		...
		Logger.warn("[buildAgentMapSnapshot] Called with no task id")
		return { rootTaskId: "", generatedAt: Date.now(), nodes: [] }
	}
	...
	const subagentNodes = (rootRuns?.runs ?? []).map((run) => subagentNodeFromRecord(run, id, "root"))
	...
	const rootNode: AgentMapNode = {
		...
		parentId: null,
		kind: "root",
		title: goal ? collapseWhitespace(goal.objectiveMarkdown) : id,
...
async function readOptionalJson(filePath: string): Promise<unknown> {
	...
	try {
		...
		if (!hasErrorCode(error, "ENOENT")) {
			Logger.warn(`[buildAgentMapSnapshot] Failed to read ${filePath}`, error)
		}
		return undefined
	}
	try {
		...
		Logger.warn(`[buildAgentMapSnapshot] Malformed JSON in ${filePath}`, error)
		return undefined
	}
}
